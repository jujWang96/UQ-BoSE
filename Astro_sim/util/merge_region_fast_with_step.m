function [sets_all, log_like_all] = merge_region_fast_with_step(cx,cy,num_region, region_area, ...
...
for t = 1:num_region-1
    ...
    tmp{merge_region2} = [];
    sets_all{t+1} = tmp;
    %plot the region intensity of each step 
    figure
    hold on
    for i = 1:length(tmp)
        log_int = log(region_intensity(i));
        scatter(cx(tmp{i}), cy(tmp{i}), 12, log_int*ones(length(tmp{i}), 1), 'filled')
    end
    scatter(cx(tmp{merge_region1}),cy(tmp{merge_region1}),'k')
