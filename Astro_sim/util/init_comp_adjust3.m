function [cx, cy, n, DT, E, cell_log_intensity, cell_area] = init_comp_adjust3(X, bound_x, bound_y, instrument_factor,instrument_poly,count)
...
for i = 1:n
    ...
    if max_x>bound_x(2) || min_x<bound_x(1) || max_y>bound_y(2) || min_y<bound_y(1)
        ...
        %get the intersection of the voronoi cell and the instrument
            area_adjust = area(intersect(polyshape(V(R{i}, 1), V(R{i}, 2)),instrument_poly));

           cell_area(i) = area_adjust *instrument_factor(i)+(cell_area(i)-area_adjust);
