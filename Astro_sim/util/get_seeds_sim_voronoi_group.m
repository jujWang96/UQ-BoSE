function  [seeds,seeds_rej, num_s] = get_seeds_sim_voronoi_group(cx, cy,invalid, adj_mat, cell_area,P,threshold,seedsize,factor)
...
% num_s: the number of seed sets selected (excluding those rejected)
...
for i = 1:num_s
    ...
    if seed_range>max_range
        index_rej = [index_rej i];
    end
end
% remove
seeds_rej = seeds(index_rej);
seeds(index_rej) = [];
num_s = num_s-length(index_rej);
% 
% seeds_pt = {};
% num_s_pt = 0;
