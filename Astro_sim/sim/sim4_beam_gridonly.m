...
function [metrics, n_region,merge_time] = sim4_beam_gridonly(loc, radius, base_num_in_circle, factors, lambda, sample_factors, seed, gridsize,seedsize)
...
for i = 1:length(factors)
    ...
    for j = 1:length(sample_factors)
        ...
        X = sim_inhomo_Pois_const([0 1], [0 1], sample_factor * lambda, loc, radius, sample_factor * factor * base_num_in_circle, seed);
            ...
            [labeled_cells, pred_class_all, n_region(index), merge_time(index)] = sim_fit_beam_gridonly(X, ...
                factor, sample_factor, seed, false,gridsize,seedsize);
