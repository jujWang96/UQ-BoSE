...
fileID = fopen('zshape_10sim_center.txt','w');
...
hold on
plot_zshape(range_x, range_y,lw_zshape,colr)
box on 
set(gca, 'fontsize', 12)
...
fprintf(fileID,'\n');
% writematrix(ratio_seg,'zshape_10sim.txt','WriteMode','append')
% writematrix(area_seg,'zshape_10sim.txt','WriteMode','append')
% writematrix(flux_seg,'zshape_10sim.txt','WriteMode','append')
...
%     % drop empty entries in the cell array
    ...
    for j = 1:num_nonempty
        area_seg = [area_seg, sum(cell_area(selected_nonempty{j}))];
        flux_seg = [flux_seg, length(selected_nonempty{j})];
    ...
    saveas(gca, strcat('zshape_sim',num2str(ii),'.png'))
