...
%adjust for the area also but not just the intensity, adjust based on the
%position of whole cell but not the just the photon
clear
close all
...
scatter(X(:, 1), X(:, 2), 'k.')
hold on
plot([instrument(:,1);instrument(1,1)],[instrument(:,2);instrument(1,2)],'LineWidth',lw)
% plot instrument
axis([0 1 0 1])
axis square
box on
...
[seeds, seeds_rej, seeds_pt, num_s, num_s_pt] = get_seeds_sim_local_max(0.1, 0.9, 0.1, 0.9,...
    stepsize, stepsize, 5, cell_log_intensity, cell_area, cx, cy, 2, 50, 5, invalid, adj_mat);
num = num_s+num_s_pt;
...
%do another smoothing over area 
instrument_poly = polyshape(instrument(:,1),instrument(:,2));
[cx, cy, n, DT, E, cell_log_intensity, cell_area] = init_comp_adjust3(X, [0 1], [0 1],nbr_prop*instrument_factor, instrument_poly,ones(size(X, 1), 1));
adj_mat = get_adj_mat( E, n );
...
%         text(0.7,0.5,num2str(log_like_all(index_BIC)))
