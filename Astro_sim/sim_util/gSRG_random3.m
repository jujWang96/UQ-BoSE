function [glb_contour,glb_region_intensity,glb_selected,glb_min_BIC,seg_contour,seg_region_intensity,seg_selected,seg_min_BIC] = gSRG_random3(X,draw_s,draw_c, Color,N_r,bound,drop,rand_num,rand_itr,itr_num,m,grid_size,seed_size) 
...
[seeds, seeds_rej, seeds_pt, num_s, num_s_pt] = get_seeds_sim_local_max(0.1, 0.9, 0.1, 0.9,...
    grid_size, grid_size, seed_size, cell_log_intensity, cell_area, cx, cy, 2, 50, seed_size, invalid,adj_mat);
num = num_s+num_s_pt;
frame = [0 1 0 1];
pt_size = 0.2;
% plot the seeds
...
for itr = 1:itr_num   
    ...
    hole_num = zeros(length(log_like_all),1);
    for k = 1:length(log_like_all)
        hole_num(k) = calc_hole(sets_all,V,R,adj_mat,invalid);
    end
    BIC_all = BIC_all-hole_num*log(n);
