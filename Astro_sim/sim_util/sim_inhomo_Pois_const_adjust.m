function X = sim_inhomo_Pois_const_adjust(range_x, range_y, lambda, loc, radius, num, factor,pos,seed, show_plot)
...
X = sim_homo_Pois_adjust(range_x, range_y, lambda,factor,pos,seed);
...
for i = 1:n_s
    ...
    % update X
    inp = inpolygon(rand_num(:, 1), rand_num(:,2),pos(:,1),pos(:,2));
    drop = binornd(1,(1-factor)*ones(1,length(inp)))';
    rand_num(find(inp.*drop==1),:) = [];
    X = [X; rand_num];
end
