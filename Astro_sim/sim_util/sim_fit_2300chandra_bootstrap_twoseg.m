...
function [fourier_coef, contourx,contoury, area_poly,area_fd, num_B_poly, num_B_fd, intensity_B_poly,...
    ...
    centx_poly, centy_poly, centx_fd, centy_fd] = sim_fit_2300chandra_bootstrap_twoseg(X, seed, show_plot, P, threshold, rand_num, rep_itr, X_obs, min_modelselect,penalty, M,origin_invx,origin_invy)
X = unique(X,'rows');
area_poly = 0; 
area_fd = 0;
num_B_poly = 0; 
...
gca = subplot(n_plot/2,2,idx_plot)
...
axis square
set(gcf, 'Position', [50 50 1000 1600]); %
