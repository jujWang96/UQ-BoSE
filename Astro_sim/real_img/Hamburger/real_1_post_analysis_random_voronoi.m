...
for i = 1:length(selected_nonempty)
    log_int = adjust_ratio+log(sum(exp(cell_log_intensity(selected_nonempty{i})).*cell_area(selected_nonempty{i}))/sum(cell_area(selected_nonempty{i})));
