...
parfor seed = 1:B 
    bootstrap_X_set_AIC = sim_fix_data_vor_fast(X,cell_area, n, DT,2,1,seed);
    ...
    centx_poly_all_AIC(seed), centy_poly_all_AIC(seed), centx_fd_all_AIC(seed), centy_fd_all_AIC(seed)] = sim_fit_random_real_data_twoseg(bootstrap_X_set_AIC{1}, seed, false, P, threshold, ...
    rand_num, rep_itr, X, min_select_AIC,penalty,M);
    bootstrap_X_set_BIC = sim_fix_data_vor_fast(X,cell_area, n, DT,2,1,seed);
    ...
    centx_poly_all_BIC(seed), centy_poly_all_BIC(seed), centx_fd_all_BIC(seed), centy_fd_all_BIC(seed)] = sim_fit_random_real_data_twoseg(bootstrap_X_set_BIC{1}, seed, false, P, threshold, ...
    rand_num, rep_itr, X, min_select_BIC,penalty,M);

end

save('bootstrap_result_299XMM_vor.mat')