...
load('tune_param_2300XMM.mat')
load('ngc2300_MOS1_evt_0.5-8.0keV_scaled.mat')
load('bootstrap_result_2300XMM.mat')
load('model_select_result_2300XMM.mat')
target_idx = 3;
imagename = 'NGC2300XMM_';
...
figure;
make_plot_GCR_color(X,fourier_coef',fourier_coef_all_AIC',[1,2],min_pair_num{1}(3),1000,r,colors);
gca = image_rescaling(gca,'NGC2300XMMGCR');
saveas(gcf, strcat(imagename, 'GCR_AIC'),'epsc')

figure;
make_plot_GCR_color(X,fourier_coef',fourier_coef_all_BIC',[1,2],min_pair_num{2}(3),1000,r,colors);
gca = image_rescaling(gca,'NGC2300XMMGCR');
...
fprintf("The 5 percentile of bootstrap flux %f\n",prctile(flux_bic{1},5))
fprintf("The 95 percentile of bootstrap flux %f\n",prctile(flux_bic{1},95))
fprintf("The 5 percentile of bootstrap bic flux %f\n",prctile(flux_bic{2},5))
fprintf("The 95 percentile of bootstrap bic flux %f\n",prctile(flux_bic{2},95) )
...
figure
make_cdf_stack(flux_bic,[num_in_poly,num_in_fd], ...
    {'bootstrap value','bootstrap BIC value','obeserved value','observerd AIC value'}, ...
    'northwest',linetypes,linecolors,20,lwidth,fontsize,drop,legdsize)
xlim([3000,3500])
xlabel('Flux')
ylabel('F(Flux)')
...
make_freq_stack(area_aic,[area_poly,area_fd], ...
    ...
    'northeast',linetypes,linecolors,binnum,lwidth,fontsize, drop,legdsize,relfreq,41)
xticks(xlimit)
...
xlimit = [2500,3800];
xticks(linspace(xlimit(1),xlimit(2),8)/(24*24*100))
xticklabels(round(linspace(xlimit(1),xlimit(2),8)))
xlim(xlimit/(24*24*100))
xlabel('Area')
ylabel('F(Area)')
set(gcf, 'Position', [50 50 600 400]); %
saveas(gcf, strcat(imagename, 'bootstrap_area_cdf_AIC'),'epsc')
fprintf("The 5 percentile of bootstrap area %f\n",prctile(area_poly_all_AIC,5)*(24*24*100))
fprintf("The 95 percentile of bootstrap area %f\n",prctile(area_poly_all_AIC,95)*(24*24*100))
fprintf("The 5 percentile of bootstrap aic area %f\n",prctile(area_fd_all_AIC,5)*(24*24*100))
fprintf("The 95 percentile of bootstrap aic area %f\n",prctile(area_fd_all_AIC,95)*(24*24*100))
...
make_freq_stack(area_bic,[area_poly,area_fd], ...
    ...
    'east',linetypes,linecolors,binnum,lwidth, fontsize,drop,legdsize,relfreq,51)
xticks(xlimit)
xticks(xlimit/(24*24*100))
...
fprintf("The 5 percentile of bootstrap area %f\n",prctile(area_poly_all_BIC,5)*(24*24*100))
fprintf("The 95 percentile of bootstrap area %f\n",prctile(area_poly_all_BIC,95)*(24*24*100))
fprintf("The 5 percentile of bootstrap bic area %f\n",prctile(area_fd_all_BIC,5)*(24*24*100))
fprintf("The 95 percentile of bootstrap bic area %f\n",prctile(area_fd_all_BIC,95)*(24*24*100))
%%
...
figure
fourier_coef_all_AIC_rescale = rescale_coef(fourier_coef_all_AIC',0,0,24,24);
...
fourier_coef_rescale = rescale_coef(fourier_coef,0,0,24,24);
...
make_eccentricity_plot([long_axis,short_axis],[long_all_axis;short_all_axis]',20,50, 'NGC2300XMM', 0.5)
saveas(gcf, strcat(imagename, 'eccentricity_scatter_AIC'),'epsc')


figure
fourier_coef_all_BIC_rescale = rescale_coef(fourier_coef_all_BIC',0,0,24,24);
