...
load('tune_param_result_2300chandra_debug.mat')
X = double(unique(X,'rows'));

imagename = 'NGC2300chandra_tune_param_';
target_idx = 4;
...
invalid_idx = num_B_poly_all_AIC==0;
num_B_poly_all_AIC(invalid_idx) = [];
num_B_fd_all_AIC(invalid_idx) = [];
num_B_fd_all_BIC(invalid_idx) = [];
num_O_poly_all_AIC(invalid_idx) = [];
num_O_fd_all_AIC(invalid_idx) = [];
num_O_fd_all_BIC(invalid_idx) = [];
area_poly_all_AIC(invalid_idx) = [];
area_fd_all_AIC(invalid_idx) = [];
area_fd_all_BIC(invalid_idx) = [];
...
long_axis = abs(abs(fourier_coef_rescale(2))+abs(fourier_coef_rescale(end))); 
short_axis = abs(abs(fourier_coef_rescale(2))-abs(fourier_coef_rescale(end))); 
make_eccentricity_plot_with_hist([long_axis,short_axis],[long_all_axis;short_all_axis]',10,50, 'NGC2300',0.5)
