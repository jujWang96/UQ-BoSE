...
load('bootstrap_result_2300XMM_vor_withseed.mat')
load('model_select_result_2300XMM.mat')
target_idx = 3;
X = double(unique(X,'rows'));
imagename = 'NGC2300XMM_vor_withseed_';
...
xlimit = linspace(1000/convertrate,12000/convertrate,21);
area_poly = polyarea(target(:,1),target(:,2));
[invx,invy] = iFD(fourier_coef',min_select_AIC);
area_aic = polyarea(invx,invy);
[invx,invy] = iFD(fourier_coef',min_select_BIC);
...
gca = image_rescaling(gca,'NGC2300XMMcentroid');
...
figure
fourier_coef_all_rescale = rescale_coef(fourier_coef_all',0,0,24,24);
...
make_eccentricity_plot_with_hist([long_axis,short_axis],[long_all_axis;short_all_axis]',20,60, 'NGC2300XMM','boxplot')
saveas(gcf, strcat(imagename, 'eccentricity_scatter_box'),'epsc')
make_eccentricity_plot_with_hist([long_axis,short_axis],[long_all_axis;short_all_axis]',20,60, 'NGC2300XMM','histogram')
