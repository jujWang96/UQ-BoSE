...
[log_like_all_buffer2, sets_all_buffer2, sets_all2, log_like_all2,~,~,~] = merge_region_fast_beamsearch_optimization(num, ...
    region_area, region_intensity, region_sets, adj_mat_region, region_num_cells, n,bw);
disp(strcat("The time used for fast beam region merging is",num2str(toc)))


%figure
%plot(log_like_all, '-o')

BIC_all2 = -2*log_like_all2+6*(num-1:-1:0)'*log(n);
[min_BIC2, index_BIC2] = min(BIC_all2);

fig = figure;
plot(num-1:-1:0, BIC_all2, '-o', 'MarkerSize', 3)
saveas(fig, 'BIC', 'png')
selected = sets_all2{index_BIC2};
