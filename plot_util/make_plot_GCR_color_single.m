function [] = make_plot_GCR_color_single(X,origin_coef,boot_F_coef,gamma,min_modelselect,n_t,r,colors)
rgb_combine = cat(3,  ones(r,r), ones(r,r),ones(r,r));
if nargin == 7
    colors = [[0.8,1,0.8];[0.8,0.8,1]];
end
for i = length(gamma):-1:1
    CI = tubeCI_single(origin_coef,boot_F_coef,n_t,min_modelselect,chi2cdf(gamma(i)^2,2),r);
    [row,col ] = find(CI==1);
    for j = 1:length(row)
