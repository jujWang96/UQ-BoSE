...
function [smooth_polyset,min_pair_num] = model_selection_multiple(X,target_x,target_y,structure,out_bd_x,out_bd_y,IC,M_pairs)
%perform the OPTIMIZED model selection for FDs of multiple target curves assuming that the target
...
tic
for l = 1:L
    layer_area = zeros(1,prod(M_pairs(structure{l})));
    layer_num = zeros(1,prod(M_pairs(structure{l})));
   
    for idx = 1:prod(M_pairs(structure{l}))
        %constructure the smoothed polygon of cooresponding number of FD
        %pairs
        clear smooth_x smooth_y

        mat_idx = get_mat_index(idx,M_pairs(structure{l}));
        for n_curve = 1:length(mat_idx)
            [smooth_x{n_curve},smooth_y{n_curve}]=iFD(f_coefs(:,structure{l}(n_curve)),mat_idx(n_curve)*2+1);
        end
        if l<L     
...
for idx = 1:prod(M_pairs)
        ...
        for IC_idx = 1:length(IC)
            if strcmp(IC(IC_idx), 'BIC')
                val = (-2)*log_like+log(n)*2*(2*sum(mat_idx)+length(mat_idx));
            elseif strcmp(IC(IC_idx), 'AIC')
                val = (-2)*log_like+2*2*(2*sum(mat_idx)+length(mat_idx));
