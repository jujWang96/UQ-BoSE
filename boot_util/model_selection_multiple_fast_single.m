...
function [smooth_polyset,min_num] = model_selection_multiple_fast_single(X,target_x,target_y,structure,out_bd_x,out_bd_y,IC,Ms)
...
for l = 1:L
    ...
    layer_area = zeros(1,prod(Ms(structure_l)));
    layer_num = zeros(1,prod(Ms(structure_l)));
    M_pair = Ms(structure_l);

    parfor idx = 1:prod(Ms(structure_l))
        ...
        for n_curve = 1:length(mat_idx)
            [smooth_x{n_curve},smooth_y{n_curve}]=iFD(f_coefs(:,structure_l(n_curve)),mat_idx(n_curve));
        end
        if l<L     
...
min_num = cell(1, length(IC));
for idx = 1:prod(Ms)
        log_like = -sum(log(1:n))-n;
        
        %decompose to the pair of each boundary
        mat_idx = get_mat_index(idx,Ms);
        for l =1:L
            arr_idx = get_arr_index(mat_idx(structure{l}),Ms(structure{l}));
        ...
        for IC_idx = 1:length(IC)
            if strcmp(IC(IC_idx), 'BIC')
                val = (-2)*log_like+log(n)*(2*sum(mat_idx)+1);
                disp(strcat("mat_idx:", num2str(sum(mat_idx)), "log_like:", num2str(log_like),";BIC:",num2str(val)))

            elseif strcmp(IC(IC_idx), 'AIC')
                val = (-2)*log_like+2*(2*sum(mat_idx)+1);
...
for IC_idx = 1:length(IC)
    ...
    for l = 1:L
        ...
        for n_curve = 1:length(structure{l})    
            [smooth_x{n_curve},smooth_y{n_curve}]=iFD(f_coefs(:,structure{l}(n_curve)),min_num{IC_idx}(structure{l}(n_curve))*2+1);
            if l<L     
 ...
 function mat_idx = get_mat_index(arr_idx,Ms)
    %given the 1-d array index and the size of matrix, return the index in
    %the multiple dimension array
        mat_idx = ones(1,numel(Ms));
        curr_dim = numel(Ms);
        while curr_dim>1
            if arr_idx>prod(Ms(1:(curr_dim-1)))
                if mod(arr_idx,prod(Ms(1:(curr_dim-1))))~=0
                    mat_idx(curr_dim) = mat_idx(curr_dim)+floor(arr_idx/prod(Ms(1:(curr_dim-1))));
                   
                    arr_idx = mod(arr_idx,prod(Ms(1:(curr_dim-1))));
                else
                    mat_idx(curr_dim) = mat_idx(curr_dim)+floor(arr_idx/prod(Ms(1:(curr_dim-1))))-1;
                    arr_idx=prod(Ms(1:(curr_dim-1)));
                end
            end
            curr_dim = curr_dim-1;  
        end
