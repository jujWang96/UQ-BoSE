function [init_sets, labeled_cells] = SRG_graph_adjust( init_sets, cell_log_intensity, cell_area, n, adj_mat, invalid,factor,adjust_indc, verbose, print_n )
% Graph-based seeded region growing, adjust for the instruments.
%
% Args:
...
%   adjust_indc: indicator of whether the photon's intensity should be
%   adjus
%
% Returns:
...
region_log_intensity_adjust = zeros(1,m); % log intensity adjusted for the instrutments 
...
for j = 1:m
    ...
    %region_log_intensity_adjust(j) = region_log_intensity(j);
    if adjust_indc(j)==1
        region_log_intensity(j) = region_log_intensity(j)- log(factor);
    end
end
%adjust for 
cell_log_intensity = cell_log_intensity-log(factor)*adjust_indc;
